...
// Constructor via SwapChain (for backward compatibility and direct usage)
...
// Constructor via explicit formats (for RenderTarget / Offscreen)
...
void GraphicsPipeline::createPipeline(const Shader& vertShader, const Shader& fragShader, const EngineConfig& config, 
    ...
    auto result = m_context.getDevice().createGraphicsPipeline(m_context.getPipelineCache(), pipelineInfo);
