...
Renderer::Renderer(VulkanContext& context, Window& window, const EngineConfig& config)
    ...
    // Fix: Move unique_ptr to shared_ptr properly
...
void Renderer::createCopyPipeline() {
    ...
    // Désactiver le culling pour le fullscreen quad
    EngineConfig copyConfig = m_config;
    copyConfig.rasterizer.setCullMode("None");

    m_copyPipeline = CreateScope<GraphicsPipeline>(m_context, 
        m_swapChain->getImageFormat(), vk::Format::eUndefined, 
        *m_shaders["fullscreen.vert"], *m_shaders["copy.frag"],
        copyConfig, std::vector<vk::DescriptorSetLayout>{m_copyLayout}, std::vector<vk::PushConstantRange>{}, 
        false, false); 
...
void Renderer::onResize(int width, int height) {
    ...
    // RecrÃ©er les sÃ©maphores de rendu si le nombre d'images a changÃ©
...
void Renderer::renderSkybox(vk::CommandBuffer cb, Scene& scene) {
    ...
    if (!skySphereView.empty()) {
        auto entity = skySphereView.front();
        auto& sky = skySphereView.get<SkySphereComponent>(entity);
        if (sky.texture) {
            m_internalSkySphereMat->setTexture(sky.texture);
            auto& pipeline = m_pipelines[MaterialType::SkySphere];
            pipeline->bind(cb);
            cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipeline->getLayout(), 0, 1, &m_globalDescriptorSets[m_currentFrame], 0, nullptr);
    ...
    if (scene.getSkybox()) {
        m_internalSkyboxMat->setCubemap(scene.getSkybox());
        auto& pipeline = m_pipelines[MaterialType::Skybox];
        pipeline->bind(cb);
        cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipeline->getLayout(), 0, 1, &m_globalDescriptorSets[m_currentFrame], 0, nullptr);
...
void Renderer::compositeToSwapchain(vk::CommandBuffer cb, uint32_t imageIndex) {
    ...
    // Pas d'attachment de profondeur ici car le pipeline de copie n'en a pas
    ...
    // Transition finale Swapchain -> Present
    vk::ImageMemoryBarrier presentBarrier(vk::AccessFlagBits::eColorAttachmentWrite, {}, vk::ImageLayout::eColorAttachmentOptimal, vk::ImageLayout::ePresentSrcKHR, VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED, swapImage, { vk::ImageAspectFlagBits::eColor, 0, 1, 0, 1 });
    cb.pipelineBarrier(vk::PipelineStageFlagBits::eColorAttachmentOutput, vk::PipelineStageFlagBits::eBottomOfPipe, {}, nullptr, nullptr, presentBarrier);
