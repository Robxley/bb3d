...
| B4 | `Renderer.cpp:1045-1066, 1082-1094` | **MEDIUM** | Au resize du picking, `m_pickingDescriptorSets.clear()` (1047) libère le vecteur mais **pas** les `vk::DescriptorSet` sous-jacents du pool (`eFreeDescriptorSet`). Chaque resize fuit `MAX_FRAMES_IN_FLIGHT` sets ; les resizes répétés épuisent le pool de 2000 sets → échec d'allocation. | Appeler `dev.freeDescriptorSets(m_descriptorPool, m_pickingDescriptorSets)` avant le `clear()`, ou abandonner `eFreeDescriptorSet` et resetter le pool entier. |
| B5 | `Renderer.cpp:450-453, 1071-1094` | **MEDIUM** | `renderEntityIds` est appelé en milieu de command buffer (cb déjà begun en 437). À la première frame / au resize, `createPickingResources()` appelle `dev.waitIdle()` (1085) **pendant l'enregistrement** — stall GPU + allocation de pipeline en plein frame. | Créer les ressources picking à l'init du Renderer ou aux limites de frame, hors `cb.begin()`. |
| B6 | `Renderer.cpp:1196-1238` | **MEDIUM** | `readEntityIdAt` alloue depuis `m_commandPool`, submit sans fence, puis `waitIdle()` — stall complet de queue par requête de picking, sur le thread principal. Le pool est partagé avec les command buffers per-frame (fragile). | Utiliser un pool transitoire dédié pour le readback, ou pipeliner le readback (copie ce frame, lecture la frame suivante avec 1 frame de latence). |
