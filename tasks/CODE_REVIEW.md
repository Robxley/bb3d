...
| B10 | `Renderer.cpp:360` | **LOW** | **[RÉSOLU]** `getMaterialForTexture` supprimé (mort, voir D4). |
...
| B14 | `PhysicsWorld.cpp:127` | **MEDIUM** | **[RÉSOLU]** `(int)hardware_concurrency() - 1` clamé à `std::max(1, ...)` pour éviter tout crash sur environnement 0/1 cœur. |
| B15 | `PhysicsWorld.cpp:294-298` | **MEDIUM** | **[RÉSOLU]** Garde `if (!entity.has<TransformComponent>()) return;` ajoutée dans `createRigidBody`. |
| B16 | `PhysicsWorld.cpp:375-376` | **MEDIUM** | **[RÉSOLU]** Vérification explicite `if (!body) return;` après `CreateBody` avant tout appel à `GetID()`. |
...
| B20 | `PhysicsWorld.cpp:397` | **MEDIUM** | **[RÉSOLU]** Garde `has<TransformComponent>()` ajoutée dans `createCharacterController` et sécurisation de la boucle update. |

### 2.3 Scène / JobSystem / Ressources

| ID | Fichier:Ligne | Gravité | Description | Direction de correction |
|----|---------------|---------|-------------|--------------------------|
| B21 | `Scene.cpp:154-166` | **MEDIUM** | **[RÉSOLU]** `Scene::destroyEntity` appelle désormais `destroyRigidBody` et `destroyCharacterController` avant suppression EnTT. |
...
| B24 | `JobSystem.cpp:104-108` | **MEDIUM** | **[RÉSOLU]** Architecture hybride Spin-Then-Park : micro-pause `_mm_pause()` dans le hot-path et `wait()` réactif sans timeout ni busy-poll au repos. |
| B25 | `JobSystem.cpp:148-151` | **LOW** | **[RÉSOLU]** `m_callerIndex` rendu membre d'instance non-statique de `JobSystem`. |
...
| D3 | `Renderer.hpp:230`, `Renderer.cpp:24,67,717` | **LOW** | **[RÉSOLU]** `m_instanceTransforms` supprimé de `Renderer.hpp` et `Renderer.cpp`. |
| D4 | `Renderer.cpp:357-364` + `Renderer.hpp:138` | **MEDIUM** | **[RÉSOLU]** `getMaterialForTexture` et `m_defaultMaterials` supprimés de `Renderer.hpp` et `Renderer.cpp`. |
| D5 | `Scene.cpp:361-399` | **MEDIUM** | **[RÉSOLU]** Bloc horizon culling commenté et `faceDirections` supprimés de `Scene.cpp`. |
| ND1 | `StagingBuffer.cpp:49-56` + `StagingBuffer.hpp:32` | **LOW** | **[RÉSOLU]** `StagingBuffer::submitCopy` (mort et buggué) et `#include <functional>` orphelin supprimés. |
