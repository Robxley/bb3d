...
static void AstroBazardLogic(Entity e, float dt) {
    ...
    if (!state.smartCamera || state.smartCamera->getTarget() != ship || state.smartCamera->getCamera() != shipCamera) {
    ...
    if (tabIsPressed && !state.tabWasPressed) {
        ...
        BB_CORE_INFO("AstroBazard: Switched camera to {}", shipCam.active ? "Spaceship Follow" : "Planet Orbit");
    }
    ...
    if (shipCamera.get<CameraComponent>().active) {
        ...
        float scaleFactor = state.smartCamera->update(altitude, gravityDir, offset);
        state.shipCtrl->applyVisualScale(ship, scaleFactor);
    } else {
        state.shipCtrl->applyVisualScale(ship, 1.0f); 
    }
}
...
void SetupScene(Engine* engine) {
    ...
    ship.get<TransformComponent>().translation = {0.0f, 13.0f, 0.0f};
    ...
    phys.initialLinearVelocity = {0.0f, 1.0f, 0.0f};
